...
class App:
    ...
    def on_event(self, event):
        ...
        if event.type == pygame.KEYDOWN:
            ...
            if action in Action.ALL_ACTIONS:
                ...
                #     for _ in range(2):
                #         action = Action.STAY
                #         done, next_state = self.step_env(action, prev_action)
                #         if done:
                #             break
    ...
    def step_env(self, my_action):
       ...
       #     joint_action = (my_action, agent2_action)
        ...
        #from IPython import embed
        #embed()
        if my_action == Action.STAY:
          self.env.state.players[1].active_log += [0]
        else:
          #if the user just stayed in place without interacting 
          if self.env.state.player_positions[1] == self.prev_pos and my_action != Action.INTERACT:
            self.env.state.players[1].active_log += [0]
          else:
            self.env.state.players[1].active_log += [1]

        self.prev_pos = self.env.state.player_positions[1]

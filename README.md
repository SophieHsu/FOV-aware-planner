...
The solver is defined in `overcooked_ai_pcg/mip_repair.py`.
...
### Generate level using trained GAN
To use trained GAN to generate Overcooked-AI levels, run the following:
...
The program will generate a level from random latent vector sampled from normal distribution and then use MILP solver defined in `overcooked_ai_pcg/mip_repair.py` to fix the level.
...
Note: These are also the constraints that the MIP solver is trying to satisfy.

### Overcooked-AI-PCG Code structure Overview

`overcooked_ai_pcg` contains:

`mip_repair.py`: Mixed Integer Linear Programming solver to fix levels generated by GAN.

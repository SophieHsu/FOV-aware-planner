...
from overcooked_ai_py.agents.agent import (HRLTrainingAgent, GreedyHumanModel, StayAgent, RandomAgent)
...
def setup_env_w_agents(config, n_epi=None, env_list=None, human=None, q=None):
    ...
    if human_type == "greedy_agent":
        ...
        human_agent = GreedyHumanModel(mlp_planner, auto_unstuck=human_config["auto_unstuck"])
    ...
    mlp_planner = MediumLevelPlanner(mdp, env_config["planner"])
    ai_agent = GreedyHumanModel(mlp_planner, auto_unstuck=human_config["auto_unstuck"])
...
def get_env_info(t_obj, state, mdp):
    ...
    t_obj['order_list'] = 0 if state.order_list is None else len(state.order_list)
...
def reset_env_from_mdp(mdp, config):
    ...
    env = OvercookedEnv.from_mdp(mdp, info_level=0, horizon=env_config["horizon"])
    return env
...
def main(dask_client, config):
    ...
    log_path = os.path.join(config["Experiment"]["log_dir"], config["Experiment"]["log_name"])
    ...
    for completion in evaluations:
        ...
        try:
            ...
            insert_log(log_path, config["Experiment"]["log_name"], completed_evals, log)
            completed_evals += 1
